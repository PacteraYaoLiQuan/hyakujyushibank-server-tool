 ...
 */
@Service
public class UseUserDownLoadCsvService
		...
		for (int i = 0; i < useUserDownLoadCsvButtonReqVO.getCsvList().size(); i++) {
			...
			if (useUserDownLoadCsvButtonReqVO.getCsvList().get(i).getSelect() == true) {
				...
				try {
					...
					followInfoList = repositoryUtil.getView(db,
							ApplicationKeys.INSIGHTVIEW_FOLLOWINFOLIST_FOLLOWINFO_USERID, FollowInfo.class, queryKey);
				...
				if (followInfoList != null && followInfoList.size() != 0) {
					for (FollowInfo followInfo : followInfoList) {
						...
						// トピック名
						useUserDownLoadInitVO.setTopicTitle(encryptorUtil.decrypt(followInfo.getTopicTitle()));
					...
					// トピック名
					useUserDownLoadInitVO.setTopicTitle("");
					downLoadList.add(useUserDownLoadInitVO);
				}
			}
		}

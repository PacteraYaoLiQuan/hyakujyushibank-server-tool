...
@Service
public class DocumentStatusModifyService extends AbstractBLogic<StatusModifyReqVO, StatusModifyResVO> {
    @Autowired
    EncryptorUtil encryptorUtil;

    @Override
    protected void preExecute(StatusModifyReqVO input) throws Exception {

    }

    @Override
    protected StatusModifyResVO doExecute(CloudantClient client, StatusModifyReqVO input) throws Exception {
        // データベースを取得
        ...
        String queryKey = input.getAccountAppSeq();
        ...
        try {
            userInfoDoc = repositoryUtil.find(db, input.getUserId(), HyakujyushiUserInfoDoc.class);
        } catch (Exception e) {

        }
        for (StatusModifyDoc yamagataStatusModifyDoc : yamagataStatusModifyList) {
            DocumentPushrecordDoc yamagataPushrecordDoc = null;
            if (!yamagataStatusModifyDoc.getPushRecordOid().isEmpty()) {
                try {
                    yamagataPushrecordDoc = (DocumentPushrecordDoc) repositoryUtil.find(db,
                            yamagataStatusModifyDoc.getPushRecordOid(), DocumentPushrecordDoc.class);
                } catch (Exception e) {
            ...
            yamagataStatusModifyListVO
                    .setStatusModifyDateForSort(String.valueOf(yamagataStatusModifyDoc.getStatusModifyDateForSort()));
            // 配信状态

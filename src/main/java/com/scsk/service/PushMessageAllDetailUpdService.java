...
@Service
public class PushMessageAllDetailUpdService
    ...
    public PushMessageDetailUpdateResVO iyo(CloudantClient client, Database db,
        ...
        YamagataMsgTitleDoc titleDoc = new YamagataMsgTitleDoc();
        YamagataMsgDetailDoc detailDoc = new YamagataMsgDetailDoc();
        List<IYoUserInfoDoc> userInfoDocList = new ArrayList<>();
        ...
        List<IYoUserInfoDoc> useUserInfoDocList = repositoryUtil.getView(db,
                ApplicationKeys.INSIGHTVIEW_IYOUSERINFOLIST_USERINFOLIST, IYoUserInfoDoc.class);

        for (IYoUserInfoDoc userInfoDoc : useUserInfoDocList) {
            int sum = 1;
            if (userInfoDoc != null) {
                if (userInfoDoc.getDeviceInfoList() != null) {
                    ...
                    String messageCode = pushNotifications.sendMessage(pushMessageDetailUpdateReqVO.getPushTitle(), sum,
                            deviceIdList);
                    // PUSH区分
                    if (Constants.RETURN_OK.equals(messageCode)) {

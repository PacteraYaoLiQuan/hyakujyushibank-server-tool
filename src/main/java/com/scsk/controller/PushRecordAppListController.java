...
import com.scsk.service.PushRecordAppListService;
import com.scsk.service.PushRecordAppRefuseService;
import com.scsk.service.PushRecordAppSendService;
...
public class PushRecordAppListController {

	@Autowired
	private PushRecordAppListService pushRecordAppListService;
    @Autowired
    private PushRecordAppSendService pushRecordAppSendService;
    @Autowired
    private PushRecordAppRefuseService pushRecordAppRefuseService;
    @Value("${bank_cd}")
	 ...
	 */
	@RequestMapping(value = "/push/pushRecordAppList", method = RequestMethod.GET, produces = "application/json")
	public ResponseEntity<ResponseEntityVO<PushRecordAppListInitResVO>> accountAppListInit(
		...
		try {
			// PUSH通知一覧データを検索する
		    pushRecordAppListInitResVO = (PushRecordAppListInitResVO) pushRecordAppListService.execute(pushRecordAppListInitResVO);
     ...
     * PUSH通知一括承認メソッド。
     * 
     * @param @RequestBody PushRecordAppListSendReqVO 一覧データ
     * @return ResponseEntity　戻るデータオブジェクト
     */
    @RequestMapping(value = "/push/sendButton", method = RequestMethod.POST, produces = "application/json")
    public ResponseEntity<ResponseEntityVO<PushRecordAppListSendReqVO>> sendButton(
        ...
        try {
            // 選択したデータを承認する
            sendReqVO = pushRecordAppSendService.execute(pushRecordAppListSendReqVO);
            if ("0169".equals(bank_cd)) {
                if (sendReqVO.getErrFlag().equals("0")) {
                    ResultMessages messages = ResultMessages.error().add(MessageKeys.E_PUSH001_1003);
                    resEntityBody.setMessages(messages);
                } else if (sendReqVO.getErrFlag().equals("1")) {
     ...
     * @return ResponseEntity　戻るデータオブジェクト
     */
    @RequestMapping(value = "/push/deleteButton", method = RequestMethod.POST, produces = "application/json")
    public ResponseEntity<ResponseEntityVO<PushRecordAppListSendReqVO>> deleteButton(
        ...
        try {
            // 選択したデータを承認する
            sendReqVO = pushRecordAppRefuseService.execute(pushRecordAppListSendReqVO);
           
            if ("0169".equals(bank_cd)) {
                if (sendReqVO.getErrFlag().equals("0")) {
                    ResultMessages messages = ResultMessages.error().add(MessageKeys.E_PUSH001_1007);
                    resEntityBody.setMessages(messages);
                } else if (sendReqVO.getErrFlag().equals("1")) {
